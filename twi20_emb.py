...
if __name__ == "__main__":
    ...
    # 节点重映射：确保节点ID是连续的 0..N-1，以避免SDNE等算法因非连续ID导致的维度不匹配问题
    ...
    g_new = eg.Graph()
    # 批量添加边可能更快，但EasyGraph可能不支持add_edges_from带映射，直接遍历
    # 为了效率，先收集所有边
    new_edges = []
    for u, v, _ in g.edges:
        new_edges.append((node_mapping[u], node_mapping[v]))
    g_new.add_edges(new_edges, [{} for _ in range(len(new_edges))]) # add_edges需要由(u,v)和属性组成的列表，或者直接add_edges_from
    
    # 替换 g
    g = g_new
    nodes_order = list(g.nodes) # 现在的节点是 0..N-1
    ...
    dw_path = 'graph_embs/dw_twibot20_emb.pt'
    ...
    n2v_path = 'graph_embs/n2v_twibot20_emb.pt'
    ...
    l_path = 'graph_embs/line_twibot20_emb.pt'
    ...
    sd_path = 'graph_embs/sd_twibot20_emb.pt'
    if os.path.exists(sd_path):
        ...
        # Train
        sdne_emb = model.train_model(epochs=100, bs=100, device=device) # Reduced epochs for demo if needed
